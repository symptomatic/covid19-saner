      ...
      if(get(MedicareHospitals, 'entry')){
        MedicareHospitals.entry.forEach(function(entry){
          if(get(entry, 'resource.resourceType') === "Organization"){
            Organizations.upsert({id: get(entry, 'resource.id')}, {$set: get(entry, 'resource')})
          }
        })
      }
    } else {
      ...
      if(Array.isArray(get(record, 'entry'))){
        record.entry.forEach(function(entry){
          if(get(entry, 'resource.resourceType') === "Measure"){
            Measures.upsert({id: get(entry, 'resource.id')}, {$set: get(entry, 'resource')}, {filter: false, validate: false});                            
          }
        });
      }

    } else if(get(record, 'resourceType') === "Measure"){
      Measures.upsert({id: get(record, 'id')}, {$set: record}, {filter: false, validate: false});                            
    }
  },
      ...
      if(Array.isArray(get(record, 'entry'))){
        record.entry.forEach(function(entry){
          if(get(entry, 'resource.resourceType') === "MeasureReport"){
            MeasureReports.upsert({id: get(entry, 'resource.id')}, {$set: get(entry, 'resource')}, {filter: false, validate: false});      
          }
        })
      }

    } else if(get(record, 'resourceType') === "MeasureReport"){
      MeasureReports.upsert({id: get(record, 'id')}, {$set: record}, {filter: false, validate: false});      
    }
  }
}
